...
function renderIdentity(){
  ...
  document.querySelectorAll("[data-user-department]").forEach(node=>node.textContent=user.department||"No department set");
}
...
function bindEvents(){
  document.querySelectorAll(".nav-btn").forEach(button=>button.addEventListener("click",()=>showView(button.dataset.view)));document.querySelectorAll("[data-go]").forEach(button=>button.addEventListener("click",()=>showView(button.dataset.go)));
  ...
  document.addEventListener("change",event=>{const box=event.target.closest?.("[data-select-expense]");if(box){const set=new Set(state.currentReport.selectedExpenseIds);box.checked?set.add(box.dataset.selectExpense):set.delete(box.dataset.selectExpense);state.currentReport.selectedExpenseIds=[...set];persist();renderAll();}});
  document.addEventListener("click",event=>{const edit=event.target.closest?.("[data-edit-expense]");if(edit)return editExpense(edit.dataset.editExpense);const del=event.target.closest?.("[data-delete-expense]");if(del)return deleteExpense(del.dataset.deleteExpense);const route=event.target.closest?.("[data-delete-route]");if(route){state.settings.savedRoutes=state.settings.savedRoutes.filter(r=>r.id!==route.dataset.deleteRoute);persist();renderSettings();return;}const pastCsv=event.target.closest?.("[data-past-csv]");if(pastCsv)return downloadPastCsv(pastCsv.dataset.pastCsv);const pastPdf=event.target.closest?.("[data-past-pdf]");if(pastPdf)return mockPdf();});
}

document.addEventListener("DOMContentLoaded",()=>{bindEvents();refreshDatalists();resetExpenseForm();renderAll();});
