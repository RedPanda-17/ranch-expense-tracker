...
if (window.__ranchAuthPrerequisiteGateV2) return;
window.__ranchAuthPrerequisiteGateV2 = true;
...
function syncIsBusy() {
  const text = String(q("completeSyncPill")?.textContent || "").trim().toLowerCase();
  return text === "syncing…" || text === "syncing...";
}

function addStyles() {
  if (q("ranchAuthGateStylesV2")) return;
  const style = document.createElement("style");
  style.id = "ranchAuthGateStylesV2";
  style.textContent = `
...
function showSignedIn(card) {
  ...
  if (syncIsBusy()) {
    document.documentElement.classList.add("ranch-auth-pending");
    gate.hidden = false;
    setChecking("Signed in. Loading your cloud data…", true);
    return;
  }
...
// Cover the app immediately when sign-out starts, before local data is cleared.
